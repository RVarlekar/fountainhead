...
- Transcribe EVERY row of the line-items table into lines[] (description, hsnSac, quantity, unit, rate, lineAmount as printed). Do not leave lines empty when the bill has an items table. lines[] holds only rows of goods/work with a quantity or amount — footer remarks, purpose notes ("for annual day", "Grade 1 to 8 ..."), and document references written on the bill (PR/PI/challan numbers) are NOT lines; leave them out.
- invoiceDate is the bill's OWN date — the one in the bill's date field/box (Date / Dt. / તા. / दिनांक, usually next to the bill number). NEVER take it from a rubber stamp (payment, E-NET, bank, "received on" stamps), from handwritten approval notes, or from a period like "2025-26" written anywhere — those are payment/receipt dates and academic years, not the bill date. When several dates appear, the bill's own date field wins.
- CORRECTED DATE: when the printed bill date is visibly circled or struck out AND a handwritten date is written on the bill as its correction (the accounts team marks the date they book the bill under), put that handwritten date in correctedDateHandwritten exactly as written — invoiceDate still carries the printed date. A stamp date or an unrelated scribble is NOT a correction; leave the field absent unless the handwriting clearly corrects the date.
- HANDWRITTEN FIGURES: copy each cell exactly as written, even when quantity × rate does not equal the written amount, or the lines do not sum to the written total — NEVER adjust one figure to make the arithmetic agree; the mismatch itself is information. On Gujarati/Devanagari handwriting be careful with easily-confused numerals (Gujarati ૧ (1) vs ૪ (4), ૫ (5) vs ૨ (2); Devanagari १ vs ४) — read them from the digit's actual shape, not from what would make the total work.
